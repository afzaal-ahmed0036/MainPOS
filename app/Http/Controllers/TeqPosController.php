...
class TeqPosController extends Controller
{
    ...
    public function createVoucher()
    {
        ...
        $lims_coupon_list = Coupon::where('is_active', true)->get();
        $flag = 0;
        $invoice_no = DB::table('invoice_master')->where('InvoiceNo', 'like', 'INV%')->count();
        $invoice_no = 'INV-0000' . ++$invoice_no;
    ...
    protected function payment($data, $updateInvoice = false)
    {
        ...
        if ($updateInvoice) {
            ...
            if (!empty($data['AmountPaidByCard'])) {$payments['AmountPaidByCard'] = $data['AmountPaidByCard'];}
            if (!empty($data['cheque_no'])) {$payments['cheque_no'] = $data['cheque_no'];}
