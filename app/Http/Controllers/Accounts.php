...
use App\Rules\NumericIfHasPercentage;
...
class Accounts extends Controller
{
    ...
    public function Login()
    {
        if(Session::has('UserID')){
            return redirect('Dashboard')->with('info', 'Already Logged In');
        }
    ...
    public function UserVerify(request $request)
    {
        ...
        if (count($data) > 0) {
            ...
            if ($data[0]->UserType == 'Biller') {
                ...
                } else return redirect('Dashboard')->with('success', 'Welcome to ' . session::get('CompanyName') . ' Software');
            }
        } else {
...
';
    ...
    public  function InvoiceCreate()
    {
        ...
        return view('invoice_create', compact('invoice_type', 'items', 'supplier', 'vhno', 'party'))->with('error', 'Logout Successfully.')->with('class', 'success');
    }
    ...
    public  function ItemSave(request $request)
    {
        ...
        $this->validate(
            ...
            [
                ...
                'Percentage' => [new NumericIfHasPercentage],
        ...
        try {
            ...
            if ($quantity > 0) {
                ...
                if ($request->Taxable == 'Yes') {
                    $tax = floatval((trim($request->Percentage) / 100) * $price);
                    $grandTotal = floatval($price + $tax);
                } else {
                    $tax = 0;
                    $grandTotal = $price;
                }
                ...
                $invoiceMasterID = InvoiceMaster::create($invoice_mst);
                $invoice_det = array(
                    'InvoiceMasterID' =>  $invoiceMasterID->id,
                    'InvoiceNo' => $invoice_no,
                    'ItemID' => $itemId->id,
            ...
            DB::commit();
            return redirect('Item')->with('success', 'Saved Successfully.');
        } catch (\Exception $e) {
            //throw $th;
            DB::rollBack();
            return back()->with('error', $e->getMessage());
    ...
    public  function ItemEdit($id)
    {
        ...
        try {
            ...
            $itemInvDetailData = DB::table('invoice_detail')->where('ItemID', $id)->where('InvoiceNo', 'like', '%BILL-%')->first();
            $invMasterData = DB::table('invoice_master')->where('InvoiceNo', $itemInvDetailData->InvoiceNo)->first();
            $categories = DB::table('item_category')->get();
            $lims_brand_list = Brand::where('is_active', true)->get();
            $units = Unit::get();
            $chartofaccount = DB::table('chartofaccount')->where(DB::raw('right(ChartOfAccountID,4)'), 00000)->where(DB::raw('right(ChartOfAccountID,5)'), '!=', 00000)->get();

            return view('item_edit', compact('pagetitle', 'item', 'chartofaccount', 'categories', 'lims_brand_list', 'units', 'invMasterData'));
        } catch (\Exception $e) {
            return back()->with('error', $e->getMessage());
        }
    }
    ...
    public  function ItemUpdate(request $request)
    {
        ...
        $this->validate(
            ...
            [
                'ItemName' => 'required|max:55',
                'ItemCode' => 'required|max:255|unique:item,ItemCode,' . $request->ItemID . ',ItemID',
                // 'stockQty' => 'required|numeric',
                // 'CostPrice' => 'required|numeric',
                'SellingPrice' => 'required|numeric',
                'unit_id' => 'required',
        ...
        try {
            ...
            if ($quantity > 0) {
                ...
                InvoiceMaster::where('InvoiceMasterID', $request->InvMasterID)->update($invoice_mst);
                $invoice_det = array(
                    'Qty' => $quantity,
                    'Description' => $request->ItemName,
                    'TaxPer' => $tax,
                ...
                InvoiceDetail::where('InvoiceMasterID', $request->InvMasterID)->update($invoice_det);
            }
            DB::commit();
...
<a href="' . URL('/PurchasePaymentDelete/' . $row->PurchasePaymentMasterID) . '"><i class="font-size-18 mdi mdi-trash-can-outline align-middle me-1 text-secondary"></i></a>
    ...
    public function PurchaseOrder()
    {
        ...
        return view('ebooks.purchaseorder');
    }
...

    {
        ...
        if ($request->ajax()) {
          ...
          <a href="' . URL('/PurchaseOrderView/' . $row->PurchaseOrderMasterID) . '"><i class="font-size-18 mdi mdi-eye-outline align-middle me-1 text-secondary"></i></a>
          <a href="' . URL('/PurchaseOrderEdit/' . $row->PurchaseOrderMasterID) . '"><i class="font-size-18 bx bx-pencil align-middle me-1 text-secondary"></i></a>

          <a href="' . URL('/PurchaseOrderDelete/' . $row->PurchaseOrderMasterID) . '"><i class="font-size-18 bx bx-trash align-middle me-1 text-secondary"></i></a>
          <a href="' . URL('/PurchaseOrderViewPDF/' . $row->PurchaseOrderMasterID) . '"><i class="font-size-18 me-1 mdi mdi-file-pdf-outline align-middle me-1 text-secondary"></i></a>
        ...
        return view('ebooks.purchaseorder', 'pagetitle');
    }
    ...
    public function PurchaseOrderSave(Request $request)
    {
        ...
        $purchase_master = array(
            ...
            'Grandtotal' => $request->input('Granttotal'),
            'UserID' => session::get('UserID'),
    ...
    public  function ExpenseCreate()
    {
        ...
        $items = DB::table('chartofaccount')->get();
        ...
        return view('expense.expensecreate', compact('invoice_type', 'tax', 'items', 'vhno', 'supplier', 'pagetitle', 'item', 'user'));
    }
