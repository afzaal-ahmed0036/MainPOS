...
class PosController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $invoices = DB::table('invoice_master')->where('InvoiceNo', 'like', 'INV%')->get();
            return Datatables::of($invoices)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn .= '<a href="javascript:void(0)" class="edit_brand" data-id="' . $row->id . '"><i class="bx bx-pencil align-middle me-1"></i></a> <a href="#" onclick="delete_confirm2(`InvoiceDelete`,' . $row->id . ')"><i class="bx bx-trash  align-middle me-1"></i></a>';
                    $btn .= ' </div></div>';
                    return $btn;
                })
